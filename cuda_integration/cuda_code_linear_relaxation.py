...
extern "C" __global__ void my_kernel(float* input_domain, int input_domain_n, int* layer_sizes, int layer_number, 
            float* full_weights, float* full_biases, float* results_cuda, int max_layer_size, int* activations) {
        ...
        for (int i = 0; i < layer_sizes[layer + 1]; i++) {
            ...
            if (layer < (layer_number - 2)) {
                if(activations[layer] == 1){
                    //concretize both equations to 0 without any loss of precision
                    ...
                    //information is partially lost, apply linear relaxation (lower equation and upper equation may be the same or different)
                    else if(tempVal_lower <= 0){
                        ...
                        if(tempVal_lower <= 0 && tempVal_lower_max <= 0 && tempVal_upper_min <= 0 && tempVal_upper > 0){
                            ...
                            new_equation[i * actual_input_size + (input_size * 2)] = 0;
                            new_equation[i * actual_input_size + (input_size * 2) + 1] *= relaxation_upper;
                        ...
                        //relax lower, relax upper
                            ...
                            new_equation[i * actual_input_size + (input_size * 2)] *= relaxation_lower;
                            new_equation[i * actual_input_size + (input_size * 2) + 1] *= relaxation_upper;
                            new_equation[i * actual_input_size + (input_size * 2) + 1] -= (tempVal_lower_max * relaxation_upper);
                        }
